...
import (
	...
	"github.com/dash0hq/agent-skills/evals/scenarios"
)
...
func TestBuildSelectionFullMatrixTriggerViaVersionsEnv(t *testing.T) {
	reg := scenarios.Default()
	out, err := buildSelection(reg, gatePR, []string{"evals/versions.env"}, nil)
	require.NoError(t, err)
...
func TestRunWithChangedFileExitsCleanly(t *testing.T) {
	...
	// the real evals/quarantine.yaml, so both run and nothing is excluded.
...
// evals/quarantine.yaml the run reads, and returns its root.
func gitRepoWithGoRuleChange(t *testing.T) string {
	...
	git("init", "-q")
	// The run reads evals/quarantine.yaml; mirror the repo's known-red entry.
	write("evals/quarantine.yaml", "quarantine:\n  - id: "+scenarios.GoLogsID+"\n    since: 2026-07-17\n")
