...
// evals/custom/scenarios package, where the scenarios are registered: an empty
...
func selectionRegistry(t *testing.T) *Registry {
	...
	r := NewRegistry()
	scenarios := []Scenario{
		{ID: "go-http", Skill: SkillInstrumentation, RuleFiles: []string{"skills/otel-instrumentation/rules/sdks/go.md"}, FixturePath: "evals/custom/fixtures/go-service", Smoke: true},
		{ID: "nodejs-http", Skill: SkillInstrumentation, RuleFiles: []string{"skills/otel-instrumentation/rules/sdks/nodejs.md"}, FixturePath: "evals/custom/fixtures/nodejs-service"},
...
func TestSelect(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			changed: []string{"evals/custom/fixtures/nodejs-service/Dockerfile"},
		...
		},
		{
			...
			changed: []string{"evals/custom/cmd/relay/main.go"},
			want:    []string{"go-http", "collector-pipeline", "ottl-redaction", "semconv-attributes"},
		},
		{
		...
		},
		{
			...
			changed: []string{"skills/otel-instrumentation/rules/sdks/go.md", "evals/custom/fixtures/go-service/main.go", "skills/otel-ottl/rules/redaction.md"},
...
func TestSelectFullMatrixOnly(t *testing.T) {
	...
	tests := []struct {
		...
		{"fixture path", []string{"evals/custom/fixtures/go-service/main.go"}},
		{"harness smoke", []string{"evals/custom/harness/runner.go"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
	...
	t.Run("smoke selection still picks a per-skill scenario", func(t *testing.T) {
		got := scenarioIDs(r.Select([]string{"evals/custom/harness/runner.go"}))
	...
	t.Run("full-matrix trigger includes the FullMatrixOnly scenario", func(t *testing.T) {
		require.Contains(t, scenarioIDs(r.Select([]string{"evals/custom/versions.env"})), "full-only")
	})
}
