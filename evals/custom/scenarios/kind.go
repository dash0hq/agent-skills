...
// (evals/custom/cmd/relay) as a container on that same network, so in-cluster
...
// a test greps evals/custom/fixtures/k8s to enforce that.
package scenarios
...
const (
	// kindClusterName must match the name in evals/custom/fixtures/k8s/kind-config.yaml.
	...
	kindConfigPath = "evals/custom/fixtures/k8s/kind-config.yaml"
	...
	// (evals/custom/fixtures/k8s/instrumented-go-service).
	instrumentedGoImage = "instrumented-go-service-eval:local"
	// nodejsImage is the uninstrumented Node.js fixture image
	// (evals/custom/fixtures/nodejs-service), the auto-instrumentation target.
...
// (evals/custom/cmd/evalhelper probe) and waits for it to succeed.
