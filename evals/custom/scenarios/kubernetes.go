...
type KindScenarioSpec struct {
	...
	// relative to the evals/custom/ module root; the KindFixture builds and loads
...
// already instrumented (evals/custom/fixtures/k8s/instrumented-go-service), so the
...
func K8sDownwardAPI() harness.Scenario {
	return harness.Scenario{
		...
		RequiresKind: true,
		FixturePath:  "evals/custom/fixtures/k8s/downward-api-workspace",
...
func K8sOTelOperator() harness.Scenario {
	return harness.Scenario{
		...
		RequiresKind: true,
		FixturePath:  "evals/custom/fixtures/k8s/operator-workspace",
...
func K8sHelmChart() harness.Scenario {
	return harness.Scenario{
		...
		RequiresKind: true,
		FixturePath:  "evals/custom/fixtures/k8s/helm-workspace",
...
func K8sRawManifests() harness.Scenario {
	return harness.Scenario{
		...
		RequiresKind: true,
		FixturePath:  "evals/custom/fixtures/k8s/raw-manifests-workspace",
...
func K8sDash0Operator() harness.Scenario {
	return harness.Scenario{
		...
		RequiresKind: true,
		FixturePath:  "evals/custom/fixtures/k8s/dash0-workspace",
