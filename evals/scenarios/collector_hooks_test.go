...
func fixtureHooksFor(t *testing.T, sc harness.Scenario) harness.FixtureHooks {
	...
	return fixtureHooks(NewDockerFixture(t, filepath.Join(testutil.RepoRoot(t), "evals")), sc)
}
...
// test process; see evals/examples/otelcolbin), skipping the test when it
// cannot be fetched in this environment.
func otelcolBinary(t *testing.T) string {
	t.Helper()
	otelcolOnce.Do(func() {
		versions, err := harness.LoadVersions(filepath.Join(testutil.RepoRoot(t), "evals", "versions.env"))
		if err != nil {
			otelcolErr = err
			return
		}
