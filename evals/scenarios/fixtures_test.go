...
var fixtureSourceFiles = map[string][]string{
	"evals/fixtures/browser-service": {"server.js", "static/app.js"},
	"evals/fixtures/dotnet-service":  {"Program.cs"},
	"evals/fixtures/go-service":      {"main.go"},
	"evals/fixtures/java-service":    {"src/main/java/checkout/CheckoutServer.java", "src/main/resources/application.properties"},
	"evals/fixtures/nextjs-service":  {"app/checkout/route.js", "Dockerfile"},
	"evals/fixtures/nodejs-service":  {"server.js"},
	"evals/fixtures/php-service":     {"index.php", "Dockerfile"},
	"evals/fixtures/python-service":  {"app.py"},
	"evals/fixtures/ruby-service":    {"config.ru", "Dockerfile"},
	"evals/fixtures/scala-service":   {"src/main/scala/checkout/CheckoutServer.scala"},
}
...
var nonServiceFixtures = map[string]string{
	"evals/fixtures/collector-workspace": "Collector config workspace the agent edits (U5); no service contract",
	"evals/fixtures/k8s":                 "Kubernetes manifests and workspaces the agent edits (U6); no service contract",
}
...
// exception" section of evals/fixtures/README.md). The full contract lint
...
// The fixture contract lint (see evals/fixtures/README.md): every fixture
...
func TestFixturesFollowTheContract(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(root, "evals", "fixtures"))
	require.NoError(t, err)
	for _, entry := range entries {
		if !entry.IsDir() {
			continue
		}
		path := "evals/fixtures/" + entry.Name()
...
func TestBrowserFixtureExceptionSurface(t *testing.T) {
	root := testutil.RepoRoot(t)
	dir := filepath.Join(root, "evals", "fixtures", "browser-service")
