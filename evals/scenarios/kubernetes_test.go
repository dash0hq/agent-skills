...
func TestKubernetesSelection(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			changed: []string{"evals/fixtures/k8s/raw-manifests-workspace/collector.yaml"},
...
// under evals/fixtures/k8s and requires each non-empty document to classify
// as a Kubernetes manifest under the evals/examples rules. Agent-authored
// seed stubs (comment-only files) are allowed.
func TestK8sFixtureManifestsAreValidAndClassifyAsK8s(t *testing.T) {
	root := testutil.RepoRoot(t)
	k8sDir := filepath.Join(root, "evals", "fixtures", "k8s")
	checked := 0
...
// from env files), so nothing token-like may live under evals/fixtures/k8s.
...
func TestOperatorSeedCarriesResourceAttrsPlaceholder(t *testing.T) {
	root := testutil.RepoRoot(t)
	path := filepath.Join(root, "evals", "fixtures", "k8s", "operator-workspace", "app-deployment.yaml")
...
func TestKubernetesScenarios(t *testing.T) {
	...
	root := testutil.RepoRoot(t)
	versions, err := harness.LoadVersions(filepath.Join(root, "evals", "versions.env"))
	require.NoError(t, err)
